                       ...
                       char *buffer, int buffer_size)
{
    ...
    if (root == NULL || data == NULL || bbox == NULL) goto done;

    if (cJSON_AddNumberToObject(root, "version", 1) != NULL &&
        cJSON_AddStringToObject(root, "type", "vision") != NULL &&
        cJSON_AddStringToObject(root, "device_id", message->device_id) != NULL &&
        cJSON_AddStringToObject(root, "message_id", message->message_id) != NULL &&
        cJSON_AddStringToObject(root, "timestamp", message->timestamp) != NULL &&
        cJSON_AddNumberToObject(data, "frame_id", (double)message->frame_id) != NULL &&
        cJSON_AddNumberToObject(data, "timestamp_ms",
                               (double)message->timestamp_ms) != NULL &&
        cJSON_AddNumberToObject(data, "class_id", message->class_id) != NULL &&
        cJSON_AddStringToObject(data, "class_name", message->class_name) != NULL &&
        cJSON_AddNumberToObject(data, "confidence", message->confidence) != NULL &&
        cJSON_AddNumberToObject(bbox, "x", message->x) != NULL &&
        cJSON_AddNumberToObject(bbox, "y", message->y) != NULL &&
        cJSON_AddNumberToObject(bbox, "width", message->width) != NULL &&
        cJSON_AddNumberToObject(bbox, "height", message->height) != NULL) {
